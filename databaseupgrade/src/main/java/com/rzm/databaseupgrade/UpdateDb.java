...
public class UpdateDb {
    ...
    public UpdateDb(Element ele) {
        ...
        NodeList before = ele.getElementsByTagName(SqlConstant.KEY_SQL_BEFORE);
        for (int i = 0; i < before.getLength(); i++) {
            //获取到这个节点中的文字内容
            String sql_before = before.item(i).getTextContent();
            //存入集合
            this.sqlBefores.add(sql_before);
        }
        //同上
        NodeList after = ele.getElementsByTagName(SqlConstant.KEY_SQL_AFTER);
        for (int i = 0; i < after.getLength(); i++) {
            String sql_after = after.item(i).getTextContent();
